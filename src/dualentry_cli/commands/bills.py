...
app = typer.Typer(help="Manage bills", no_args_is_help=True, cls=HelpfulGroup)
...
def list_bills(
    ...
    client = get_client()
    _do_list(client, "bills", "bill", limit, offset, all_pages, output, search=search, status=status, start_date=start_date, end_date=end_date)


@app.command("get")
def get_bill(
    number: int = typer.Argument(help="Bill number"),
    output: str = Format,
):
    ...
    client = get_client()
    data = client.get(f"/bills/{number}/")
    format_output(data, resource="bill", fmt=output)
